...
> 历史设计稿，非当前部署或代码结构说明。下文的本地文件目录、`FileStorage` 抽象、`app/api/modules` 建议目录及旧互评流程没有按此实现。当前业务路由主要在 `backend/app/main.py`，文件由 `backend/app/storage.py` 对接私有 OSS，ZIP 导出由 `backend/app/worker.py` 异步处理；部署请以 `docs/服务器直接部署说明.md` 为准。

> 文档版本：V1.1  
> 需求基线：`PRD-软件工程作业系统.md` V1.0  
> 设计范围：服务端架构；Web 客户端实现见 `docs/前端架构与原型说明.md`  
> 首版架构：FastAPI 模块化单体 + SQL Server + 本地文件存储

## 1. 设计结论

本系统首版采用简单的模块化单体，不采用微服务，也不把 Redis、Celery、MinIO、消息队列或 Kubernetes 作为必需组件。

服务端由一个 FastAPI 应用承载全部业务 API，SQL Server 保存业务数据、任务状态和实时 outbox，服务器私有目录保存作业文件。批量导出等耗时操作由同一代码库中的轻量任务进程执行。API 与任务进程共享业务代码和数据库，但以不同进程运行。
...
2. **保持部署简单**：首版只依赖 FastAPI、SQL Server 和文件目录。
...
首版只有一名教师，该教师同时承担账号维护、运行配置、失败任务处理和审计查询，不设置独立的系统管理员角色。审计日志仍不可由教师修改或删除。首版不负责统一身份认证、外部消息渠道、Office 在线编辑、教务成绩同步、代码托管和自动化测试。客户端页面、路由和状态管理在 `docs/前端架构与原型说明.md` 中定义；正式 Web 客户端采用 Vue 3 + Ant Design Vue，并由 FastAPI 提供同源入口。

## 4. 整体部署架构

```mermaid
flowchart TB
    Request["HTTP 请求"] --> API["FastAPI 应用<br/>静态页面、业务 API、gzip"]

    subgraph Server["单机运行环境"]
        Runner["任务进程<br/>轮询数据库任务表"]
        DB[("SQL Server<br/>业务数据、会话、任务")]
...
| 组件 | 职责 |
|---|---|
| FastAPI | 前端静态文件、gzip、接口、鉴权、业务规则、事务和文件访问授权 |
| SQL Server | 业务事实、会话、幂等记录、任务表、实时 outbox 和审计日志 |
| 文件目录 | 保存原始附件、派生预览和临时导出；目录不直接暴露公网 |
| 任务进程 | 执行批量导出、截止提醒和过期文件清理 |
| Office 文件 | DOCX/XLSX 不转码，由 API 校验权限后返回原文件下载 |

页面实时更新使用同源 SSE。业务事务把不含业务正文的失效事件写入 SQL Server `realtime_events` outbox，每个 API worker 独立轮询并按登录用户、角色和教学班分发。事务回滚时事件同时回滚；SSE 仍只提示客户端静默重新查询相关资源。
...
- 时间使用 ISO 8601，数据库使用 `datetimeoffset` 语义并统一保存 UTC。
...
- 会话记录保存在 SQL Server，包含用户、过期时间、认证版本和撤销时间。
...
SQL Server 中设置通用 `background_job` 表：
...
任务进程使用 SQL Server `UPDLOCK, ROWLOCK, READPAST` 表提示领取任务，跳过其他进程已锁定的行。任务处理必须幂等；失败采用有限次数的延迟重试，超过上限后保留失败状态供教师查看。API 在业务事务内同时写业务数据和任务记录，因此不需要额外 Outbox 或消息队列。
...
- OSS Bucket 保持私有；Markdown 预览先经过 API 授权并获取短期签名 URL，再由浏览器直接读取，其他下载仍由 API 授权。
...
- API 为每个请求生成或接收 `X-Request-ID`，并将其写入响应和审计记录。
...
| `/health/ready` | SQL Server、文件目录和迁移版本是否可用 |

最低监控项包括 API 错误率和响应时间、数据库连接数、磁盘剩余空间、失败任务数量、预览失败率和最近备份结果。

### 12.3 备份恢复

- 每日备份 SQL Server 和文件目录，使用相同批次号记录两者对应关系。
- 备份保存到与应用数据盘不同的位置。
...
本地开发默认仅通过 Docker Compose 运行 SQL Server 2022；FastAPI、任务进程和 Vue/Vite 在宿主机运行：

```text
Docker: SQL Server 容器 1433 -> 本机 1433，数据库兼容级别 120
本地:  FastAPI:8000 + task-runner + Vite:8080
```

自动化测试使用 `test` profile 启动独立的 SQL Server `coursework_test` 数据库、API 和前端，测试数据库使用临时文件系统且不映射宿主端口，不读写开发数据；需要排查容器部署时，可使用 `container-app` profile 启动 Nginx、API、任务进程和 SQL Server。`api` 和 `task-runner` 使用同一个应用镜像并挂载同一个私有文件卷。

生产环境在单台 Linux 服务器直接部署，不依赖 Docker：FastAPI 同时提供 API 与 Vue 3 构建产物，并启用 gzip 和静态资源缓存；API 和任务进程由 systemd 分别托管，并连接 SQL Server；私有文件目录使用服务器持久化路径。具体步骤见 `docs/服务器直接部署说明.md`。
...
- 数据库、文件和备份使用独立持久目录。
- API 校验请求体大小并使用流式上传，避免把整个文件读入内存。
...
| SQL Server 查询无法满足复杂全文检索 | 评估专用搜索引擎 |
