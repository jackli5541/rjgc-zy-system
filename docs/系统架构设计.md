...
页面实时更新使用同源 SSE。业务事务通过 PostgreSQL `NOTIFY` 发布不含业务正文的失效事件，每个 API worker 使用一条 `LISTEN` 连接接收并按登录用户、角色和教学班分发。SSE 只提示客户端静默重新查询相关资源，普通鉴权 API 始终是数据事实来源；连接中断、队列积压或浏览器恢复可见时会执行一次兜底同步。
...
生产环境在单台 Linux 服务器直接部署，不依赖 Docker：FastAPI 同时提供 API 与 Vue 3 构建产物，并启用 gzip 和静态资源缓存；API 和任务进程由 systemd 分别托管；PostgreSQL 和私有文件目录使用服务器持久化路径。具体步骤见 `docs/服务器直接部署说明.md`。
