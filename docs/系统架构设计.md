...
服务端由一个 FastAPI 应用承载全部业务 API，PostgreSQL 保存业务数据和任务状态，服务器私有目录保存作业文件。Office 预览、批量导出等耗时操作由同一代码库中的轻量任务进程执行。API 与任务进程共享业务代码和数据库，但以不同进程运行，避免转码阻塞普通请求。
...
flowchart LR
    ...
    System --> Files["私有文件目录"]
    System --> Converter["LibreOffice<br/>可选的 Office 转 PDF 工具"]
    ...
    class System app;
    class Database,Files,Converter infra;
```
...
flowchart TB
    ...
    subgraph Server["单机运行环境"]
        ...
        Office["LibreOffice Headless<br/>按需转 PDF"]

        API -->|事务读写| DB
        API -->|保存与读取| FileDir
        Runner -->|领取任务、更新状态| DB
        Runner -->|读取与生成文件| FileDir
        Runner -->|Office 文件| Office
    end
    ...
    class Request entry;
    class Nginx,API,Runner,Office service;
...
| 任务进程 | 执行转码、批量导出、截止提醒和过期文件清理 |
| LibreOffice | 将 DOCX、PPTX、XLSX 转为 PDF；失败不影响原文件下载 |
...
| 互评与成绩 | 量表、评价权限、评价作废、评分草稿、成绩发布与修改历史 |
...
sequenceDiagram
    ...
    participant Office as LibreOffice
...
- PDF 和图片可直接预览；Office 文件由任务进程调用 LibreOffice 转 PDF。
- LibreOffice 使用非特权用户、独立临时目录、执行超时和资源上限；不执行宏，不允许访问外部网络。
...
自动化测试使用 `test` profile 启动独立的 `coursework_test` 数据库、API 和前端，测试数据库使用临时文件系统且不映射宿主端口，不读写开发数据；需要排查容器部署时，可使用 `container-app` profile 启动 Nginx、API、任务进程和 PostgreSQL。`api` 和 `task-runner` 使用同一个应用镜像并挂载同一个私有文件卷。LibreOffice 可安装在任务镜像中，无 Office 预览需求的环境可以不安装。
...
| F-22 至 F-26 | 互评与成绩 | 关系校验、量表快照、原子发布和修改历史 |
...
7. 互评禁止自评、跨组和对同一人的重复有效评价。
8. 文件 URL 无法绕过服务端权限；Markdown 危险内容不能执行。
9. Office 转码失败不影响正式提交和原文件下载。
