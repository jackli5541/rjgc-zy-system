...
const MANAGED_IMAGE_URL = /^\/api\/v1\/(?:markdown-assets|teaching-material-assets|capstone-assets)\/[0-9a-f-]{36}\/content$/i
...
function secureUrls(container) {
  ...
  container.querySelectorAll('img').forEach(image => {
    ...
    if (!/^https?:\/\//i.test(src) && !MANAGED_IMAGE_URL.test(src)) image.removeAttribute('src')
...
export function renderMarkdown(source) {
  ...
  const clean = DOMPurify.sanitize(rendered, {
    ...
    ADD_ATTR: ['width', 'height', 'loading', 'decoding'],
