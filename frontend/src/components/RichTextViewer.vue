...
function focusAnnotation(id) {
  if (!id) return
  const target = root.value?.querySelector(`[data-annotation-id="${id}"]`)
  if (!target) return
...
function annotationRect(id) {
  return root.value?.querySelector(`[data-comment-annotation-id="${id}"]`)?.getBoundingClientRect() || root.value?.querySelector(`[data-annotation-id="${id}"]`)?.getBoundingClientRect() || null
}

function viewportRect() { return root.value?.parentElement?.getBoundingClientRect() || root.value?.getBoundingClientRect() || null }
