...
import { ArrowDownOutlined, ArrowUpOutlined, CaretRightOutlined, CommentOutlined, CompressOutlined, DeleteOutlined, DownloadOutlined, ExpandOutlined, HighlightOutlined, LeftOutlined, RightOutlined, StrikethroughOutlined, UnderlineOutlined } from '@ant-design/icons-vue'
...
<template>
  <a-drawer :open="open" :width="expanded?'100vw':'min(100vw, 1440px)'" :z-index="1200" placement="right" :root-class-name="expanded?'review-workspace-drawer expanded':'review-workspace-drawer'" @close="emit('close')">
    ...
    <a-tooltip :title="expanded?'收回':'展开至全屏'" placement="right"><button type="button" class="review-drawer-expand-button" :aria-label="expanded?'收回作业批改':'将作业批改展开至全屏'" @click="emit('update:expanded',!expanded)"><CompressOutlined v-if="expanded"/><ExpandOutlined v-else/></button></a-tooltip>
