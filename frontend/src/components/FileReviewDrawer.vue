...
import { CommentOutlined, DeleteOutlined, DownloadOutlined, HighlightOutlined, LeftOutlined, RightOutlined, StrikethroughOutlined, UnderlineOutlined } from '@ant-design/icons-vue'
...
const emit = defineEmits(['close', 'feedback-published', 'clear-feedback'])
const index = ref(0)
...
const commentComposer = reactive({ open: false, mode: 'new', annotationId: '', left: 0, top: 0, value: '' })
let loadSequence = 0
...
function resetFeedback(value = {}) {
  Object.assign(feedback, { status: null, revision: 0, grade: 'A', comment: '', annotations: [], published_at: null, has_draft: false }, value)
...
async function loadFeedback() {
  ...
  try { resetFeedback(await api(`/submission-versions/${props.submissionVersionId}/feedback`)) }
...
function anchoredPosition(rect, width, height) {
  ...
  const top = rect.top > height + 16 ? rect.top - height - 10 : Math.min(window.innerHeight - height - 12, rect.bottom + 10)
...
function addAnnotation(payload, markType, color, comment = '') {
  if (!props.editable || !activeFile.value) return null
  const annotation = { id: crypto.randomUUID(), file_id: activeFile.value.id, kind: payload.kind, mark_type: markType, color, anchor: payload.anchor, comment }
...
function selectAnnotation(payload, openBubble = true) {
  ...
  requestAnimationFrame(() => document.querySelector(`.annotation-item[data-annotation-id="${id}"]`)?.scrollIntoView({ block: 'nearest' }))
  ...
  if (typeof payload !== 'string' && payload.viewportRect) return showAnnotationComment(annotation, payload.viewportRect)
  requestAnimationFrame(() => requestAnimationFrame(() => {
    const bubble = document.querySelector(`[data-comment-annotation-id="${id}"]`)
    if (bubble) showAnnotationComment(annotation, bubble.getBoundingClientRect())
  }))
}
...
async function saveFeedback(publish) {
  ...
  saving.value = true
  try {
    const path = `/submission-versions/${props.submissionVersionId}/feedback/${publish ? 'publish' : 'draft'}`
    const saved = await api(path, { method: publish ? 'POST' : 'PUT', body: JSON.stringify(feedbackPayload()) })
    resetFeedback(saved)
    emit('feedback-published', saved.result)
    message.success(publish ? '教师反馈已发布' : '反馈草稿已保存')
  } catch (saveError) { message.error(saveError.message) }
...
watch(index, () => { selectedAnnotationId.value = ''; dismissTransient(); loadFile() })
watch(() => props.files, () => { if (props.open) loadFile() }, { deep: true })
onMounted(() => { document.addEventListener('pointerdown', handleGlobalPointer); document.addEventListener('scroll', handleGlobalScroll, true); document.addEventListener('keydown', handleGlobalKey) })
onBeforeUnmount(() => { loadSequence += 1; releaseBinaryUrl(); document.removeEventListener('pointerdown', handleGlobalPointer); document.removeEventListener('scroll', handleGlobalScroll, true); document.removeEventListener('keydown', handleGlobalKey) })
</script>
...
<template>
  <a-drawer :open="open" :width="'min(100vw, 1440px)'" placement="right" root-class-name="review-workspace-drawer" :title="activeFile?.name||'文件预览'" @close="emit('close')">
    ...
    <div class="review-workspace" :class="{'with-feedback':hasFeedbackPanel}">
      <main class="review-document-stage">
        ...
        <PdfDocumentViewer v-else-if="renderType==='PDF'&&binaryUrl" :url="binaryUrl" :annotations="activeAnnotations" :editable="editable" :selected-annotation-id="selectedAnnotationId" @selection="handleSelection" @select="selectAnnotation" @error="error=$event"/>
        <div v-else-if="renderType==='RICH_TEXT'&&richHtml" class="rich-document-scroll"><RichTextViewer :html="richHtml" :annotations="activeAnnotations" :editable="editable" :selected-annotation-id="selectedAnnotationId" @selection="handleSelection" @select="selectAnnotation"/></div>
      ...
      <aside v-if="hasFeedbackPanel" class="feedback-sidebar">
        ...
        <div v-if="!editable&&feedback.comment" class="published-overall"><strong>教师总评</strong><div v-html="feedback.comment"></div></div>
        <div v-if="editable" class="feedback-actions"><a-button v-if="feedback.status" danger @click="emit('clear-feedback')">清除反馈</a-button><span/><a-button :loading="saving" @click="saveFeedback(false)">保存草稿</a-button><a-button type="primary" :loading="saving" @click="saveFeedback(true)">{{feedback.status==='PUBLISHED'?'更新反馈':'发布反馈'}}</a-button></div>
      </aside>
    </div>
  </a-drawer>
  <Teleport to="body">
    ...
    <div v-if="commentComposer.open" class="annotation-composer" :style="{left:`${commentComposer.left}px`,top:`${commentComposer.top}px`}" @pointerdown.stop @keydown.ctrl.enter.prevent="submitComment" @keydown.esc.stop.prevent="cancelComment">
      <template v-if="editable">
