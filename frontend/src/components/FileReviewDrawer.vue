    ...
    <div ref="drawerRoot" class="file-review-workspace" :class="{'with-feedback':hasFeedbackPanel}">
      ...
      <aside v-if="hasFeedbackPanel" class="feedback-sidebar">
        ...
        <section class="annotation-section">
          ...
          <article v-for="(annotation,annotationIndex) in activeAnnotations" :key="annotation.id" class="annotation-item" :class="{active:selectedAnnotationId===annotation.id}" :data-sidebar-annotation-id="annotation.id" @click="selectAnnotation(annotation.id,(annotation.mark_type||(annotation.comment?'COMMENT':'HIGHLIGHT'))==='COMMENT')">
        ...
        <section v-if="mode==='TEACHER'" class="peer-feedback-section">
          <div class="peer-feedback-heading"><div><strong>学生互评</strong><span v-if="peerGrade">综合等级 {{peerGrade}}</span></div><a-tag color="purple">{{visiblePeerFeedbacks.length}} 人</a-tag></div>
          <a-empty v-if="!visiblePeerFeedbacks.length" image="simple" description="暂无学生互评"/>
          <article v-for="peer in visiblePeerFeedbacks" :key="peer.id" class="peer-feedback-card">
        ...
        <div v-if="editable" class="feedback-actions">
          <label class="feedback-action-grade"><span>作业等级</span><a-select v-model:value="feedback.grade" placeholder="请选择等级" :options="['A','B','C','D','E'].map(value=>({value,label:value}))"/></label>
