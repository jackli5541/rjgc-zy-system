...
import { AudioOutlined, BoldOutlined, CodeOutlined, DeploymentUnitOutlined, ItalicOutlined, LinkOutlined, OrderedListOutlined, PictureOutlined, StopOutlined, UndoOutlined, UnorderedListOutlined } from '@ant-design/icons-vue'
import { message } from 'ant-design-vue'

let activeSpeechStop = null
const MAX_MARKDOWN_BYTES = 500 * 1024 * 1024

const props = defineProps({ modelValue: { type: String, default: '' }, placeholder: { type: String, default: '' }, compact: Boolean, document: Boolean, autofocus: Boolean, speechEnabled: Boolean })
...
const EditableImage = Image.extend({
  ...
  addNodeView() {
    return ({ node, getPos, editor: nodeEditor }) => {
      ...
      dom.append(image, deleteButton, widthHandle, heightHandle, cornerHandle)
      return {
        dom,
        update: nextNode => {
...
const editor = useEditor({
  ...
  extensions: [StarterKit.configure({ link: { openOnClick: false } }), EditableImage.configure({ allowBase64: true }), MermaidBlock],
