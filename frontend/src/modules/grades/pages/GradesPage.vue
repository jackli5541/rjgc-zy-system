...
<template>
  ...
  <template v-if="role==='TEACHER'">
    ...
    <section class="grade-export-section grades-tabs-card">
      <a-select v-model:value="gradesTab" class="grades-mobile-view" :options="[{value:'overview',label:'成绩总览'},{value:'export',label:'单次作业导出'},{value:'data',label:'基础数据与导出'}]" aria-label="选择成绩视图"/>
      <a-tabs v-model:activeKey="gradesTab" class="grades-tabs" :animated="{inkBar:true,tabPane:true}">
        <a-tab-pane key="overview">
          ...
          <a-table class="grades-desktop-table" :data-source="overviewItems" :loading="overviewLoading" row-key="student_id" size="small" :pagination="false">
          ...
          <div class="grades-mobile-list"><article v-for="item in overviewItems" :key="item.student_id" class="grades-mobile-item"><div class="grades-mobile-heading"><div><strong>{{item.name}}</strong><small>{{item.student_no}} · {{item.team_name || '未分组'}}</small></div><a-tag v-if="item.grade" :color="gradeTagColor(item.grade)">{{item.grade}}</a-tag></div><div class="grades-mobile-scores"><span>综合 <strong>{{item.composite_score ?? '—'}}</strong></span><span>平时 <strong>{{item.routine_score ?? '—'}}</strong></span><span>实验 <strong>{{item.lab_score ?? '—'}}</strong></span><span>期末 <strong>{{item.capstone_score ?? '—'}}</strong></span></div></article><a-empty v-if="!overviewLoading&&!overviewItems.length" description="暂无成绩"/></div>
        </a-tab-pane>
