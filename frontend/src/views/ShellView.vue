...
const campaignEnded = computed(() => Boolean(selectedCampaign.value && new Date(selectedCampaign.value.due_at) <= new Date()))
  ...
  <a-layout v-if="session.user" class="app-shell">
    ...
    <a-layout-content class="app-content"><div class="content-wrap"><a-spin :spinning="loading">
      ...
      <template v-else-if="view==='reviews'">
        ...
        <a-empty v-if="!campaigns.length" description="暂无互评活动"/><template v-if="role==='STUDENT'"><a-card v-for="item in campaigns" :key="item.id" class="assignment-row" :class="campaignStateClass(item)" :bordered="false" @click="openCampaign(item)"><div class="assignment-icon cyan"><FormOutlined/></div><div class="assignment-main"><div class="assignment-heading"><h3>{{item.assignment_title}}</h3><a-tag v-if="['COMPLETED','SKIPPED'].includes(item.allocation_status)">已完成</a-tag><a-tag v-else-if="new Date(item.due_at)<=new Date()" color="error">已逾期</a-tag></div><p>{{item.criteria_text||'互评标准见附件'}}</p><div class="assignment-meta"><span>{{item.mode==='CLASS'?'班级一人评一人':'组内一人评一人'}}</span><span>截止 {{formatTime(item.due_at)}}</span></div></div><div class="assignment-end"><strong>{{new Date(item.due_at)<=new Date()?'查看结果':item.allocation_status==='COMPLETED'?'已评价':'待评价'}}</strong></div></a-card></template><template v-else><a-card v-for="item in campaigns" :key="item.id" class="review-card" :bordered="false"><div class="review-card-head"><FormOutlined class="review-symbol purple"/><div><h3>{{item.assignment_title}}</h3><span>{{item.mode==='CLASS'?'班级一人评一人':'组内一人评一人'}} · 截止 {{formatTime(item.due_at)}}</span></div><a-tag color="blue">{{statusLabel(item.status)}}</a-tag><a-button @click="openCampaign(item)">查看统计</a-button></div></a-card></template>
      </template>
    ...
    <a-modal :open="Boolean(selectedCampaign)&&role==='TEACHER'" :title="selectedCampaign?.assignment_title" :footer="null" width="760px" @cancel="selectedCampaign=null">
      ...
      <a-space class="detail-actions" wrap><a-button v-if="selectedCampaign?.status==='ACTIVE'&&new Date(selectedCampaign?.due_at)>new Date()" danger @click="closeCampaign">提前截止</a-button></a-space>
      <a-alert v-for="item in campaignStats?.skipped||[]" :key="item.reviewer_id" type="warning" :message="`${item.reviewer_name}：${item.reason}`"/>
