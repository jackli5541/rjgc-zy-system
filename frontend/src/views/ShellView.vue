...
const pendingCriteriaFiles = ref([])
...
const campaignForm = reactive({ id: '', version: 1, has_reviews: false, assignment_id: '', mode: 'TEAM', criteria_text: '', due_at: '' })
...
const view = computed(() => route.name === 'assignment-detail' ? 'assignment-detail' : route.name === 'review-detail' ? 'review-detail' : route.params.view || 'overview')
const detailId = computed(() => route.params.id)
const navView = computed(() => view.value === 'assignment-detail' ? 'assignments' : view.value === 'review-detail' ? 'reviews' : view.value)
  ...
  try {
    ...
    if (view.value === 'assignment-detail') {
      ...
      if (!assignment) { message.error('作业不存在或无权查看'); await router.replace('/assignments') }
        ...
        if (selectedAssignment.value?.id !== assignment.id) assignmentDetailTab.value = 'details'
...
function queueCriteriaAttachment(file) { pendingCriteriaFiles.value.push(file); return false }
async function createAssignment(publishRequested = false) {
  ...
  if (assignmentForm.auto_review_enabled) {
    ...
    if (!assignmentForm.auto_review_criteria_text.trim() && !pendingAutoReviewFiles.value.length) return message.warning('自动互评标准文字和附件至少提供一种')
  }
...
async function loadAssignmentDetail(item) {
  selectedAssignment.value = item; selectedSubmission.value = null; boardFilter.value = 'ALL'; boardTeamFilter.value = 'ALL'; boardQuery.value = ''
  const files = await api(`/assignments/${item.id}/files`); assignmentAttachments.value = files.attachments; reviewCriteriaFiles.value = files.review_criteria || []; draftFiles.value = files.drafts
  if (role.value === 'STUDENT') {
...
async function openCampaignCreate() {
  Object.assign(campaignForm, { id: '', version: 1, has_reviews: false, assignment_id: '', mode: 'TEAM', criteria_text: '', due_at: '' })
  pendingCriteriaFiles.value = []
  modals.campaign = true
}
async function createCampaign() {
  if (!campaignForm.assignment_id) return message.warning('请选择个人作业')
  if (!campaignForm.due_at) return message.warning('请选择截止时间')
  if (!campaignForm.criteria_text.trim() && !pendingCriteriaFiles.value.length) return message.warning('互评标准文字和附件至少提供一种')
  try {
    const criteria_file_ids = []
    for (const file of pendingCriteriaFiles.value) {
      const body = new FormData(); body.append('file', file)
      const saved = await api(`/assignments/${campaignForm.assignment_id}/files?purpose=REVIEW_CRITERIA`, { method: 'POST', body })
      criteria_file_ids.push(saved.id)
    }
    const result = await api('/review-campaigns', { method: 'POST', body: JSON.stringify({ assignment_id: campaignForm.assignment_id, mode: campaignForm.mode, criteria_text: campaignForm.criteria_text, criteria_file_ids, due_at: iso(campaignForm.due_at) }) })
    modals.campaign = false; await loadView(); message.success(`互评已创建，分配 ${result.allocated} 人`)
    if (result.warnings?.length) message.warning(result.warnings.map(item => `${item.group}：${item.reason}`).join('；'))
  } catch (e) { message.error(e.message) }
}
async function loadCampaignDetail(item) {
  selectedCampaign.value = item; reviewTask.value = null; candidates.value = []
  if (role.value === 'STUDENT') {
    if (new Date(item.due_at) > new Date()) {
      if (item.assignment_snapshot_at) {
      ...
      } else candidates.value = (await api(`/review-campaigns/${item.id}/candidates`)).items
    }
  }
  else {
...
async function openCampaign(item) {
  await router.push(`/reviews/${item.id}`)
}
function closeCampaign() {
  Modal.confirm({ title: '确认提前截止互评？', content: '截止后学生不能继续提交评价，已有评价和统计会保留。', okText: '确认截止', okType: 'danger', cancelText: '取消', onOk: () => action(async () => { const result = await api(`/review-campaigns/${selectedCampaign.value.id}/close`, { method: 'POST' }); Object.assign(selectedCampaign.value, result); await loadCampaignDetail(selectedCampaign.value) }, '互评已提前截止') })
}
function startReview(person = null) {
  if (selectedCampaign.value?.assignment_snapshot_at) Object.assign(reviewForm, { score: reviewTask.value?.review?.score || 0, comment: reviewTask.value?.review?.comment || '', reviewee_id: '', scores: {} })
  else Object.assign(reviewForm, { score: 0, reviewee_id: person.user_id, comment: person.review?.comment || '', scores: person.review?.scores || Object.fromEntries(selectedCampaign.value.rubric.map(x => [x.key, 0])) })
  modals.review = true
}
async function submitReview() {
  const payload = selectedCampaign.value?.assignment_snapshot_at ? { score: reviewForm.score, comment: reviewForm.comment } : { reviewee_id: reviewForm.reviewee_id, scores: reviewForm.scores, comment: reviewForm.comment }
  await action(async () => { await api(`/review-campaigns/${selectedCampaign.value.id}/reviews`, { method: 'POST', body: JSON.stringify(payload) }); modals.review = false; await loadCampaignDetail(selectedCampaign.value) }, '评价已提交')
}
    ...
    <a-layout-content class="app-content"><div class="content-wrap"><a-spin :spinning="loading">
      ...
      <template v-else-if="view==='assignment-detail'&&selectedAssignment">
        ...
        <section class="assignment-workspace">
          <a-tabs v-model:active-key="assignmentDetailTab" class="assignment-detail-tabs">
      ...
      <template v-else-if="view==='reviews'">
        ...
        <a-empty v-if="!campaigns.length" description="暂无互评活动"/>
        <template v-if="role==='STUDENT'">
          <a-card v-for="item in campaigns" :key="item.id" class="assignment-row" :class="campaignStateClass(item)" :bordered="false" @click="openCampaign(item)">
            <div class="assignment-icon cyan"><FormOutlined/></div>
            <div class="assignment-main"><div class="assignment-heading"><h3>{{item.assignment_title}}</h3><a-tag v-if="['COMPLETED','SKIPPED'].includes(item.allocation_status)">已完成</a-tag><a-tag v-else-if="new Date(item.due_at)<=new Date()" color="error">已逾期</a-tag></div><p>{{item.criteria_text||'互评标准见附件'}}</p><div class="assignment-meta"><span>{{item.mode==='CLASS'?'班级一人评一人':'组内一人评一人'}}</span><span>截止 {{formatTime(item.due_at)}}</span></div></div>
            <div class="assignment-end"><strong>{{new Date(item.due_at)<=new Date()?'查看结果':item.allocation_status==='COMPLETED'?'已评价':'待评价'}}</strong></div>
          </a-card>
        </template>
        <template v-else>
          <a-card v-for="item in campaigns" :key="item.id" class="assignment-row review-card" :bordered="false" @click="openCampaign(item)">
            <div class="assignment-icon cyan"><FormOutlined/></div>
            <div class="assignment-main"><div class="assignment-heading"><h3>{{item.assignment_title}}</h3></div><p>{{item.mode==='CLASS'?'班级一人评一人':'组内一人评一人'}}</p><div class="assignment-meta"><span>截止 {{formatTime(item.due_at)}}</span></div></div>
            <div class="assignment-end"><a-tag :color="item.status==='ACTIVE'?'blue':'default'">{{statusLabel(item.status)}}</a-tag><a-button type="link" @click.stop="openCampaign(item)">查看统计</a-button></div>
          </a-card>
        </template>
      </template>
      ...
      <template v-else-if="view==='review-detail'&&selectedCampaign">
        ...
        <section class="assignment-workspace review-workspace">
          <template v-if="role==='TEACHER'">
            <section class="assignment-pane teacher-submission-pane">
              <div class="assignment-pane-heading"><h2>互评概览</h2><a-button v-if="selectedCampaign.status==='ACTIVE'&&!campaignEnded" danger @click="closeCampaign">提前截止</a-button></div>
            ...
            <section class="assignment-pane">
              <div class="assignment-pane-heading"><h2>评价明细</h2><span>{{campaignReviews.length}} 条评价</span></div>
              <a-alert v-for="item in campaignStats?.skipped||[]" :key="item.reviewer_id" class="soft-alert" type="warning" :message="`${item.reviewer_name}：${item.reason}`"/>
              <a-empty v-if="!campaignReviews.length" class="detail-empty" description="暂无评价记录"/>
              <a-table v-else :data-source="campaignReviews" row-key="id" size="small" :scroll="{x:640}"><a-table-column title="评价人" data-index="reviewer_name"/><a-table-column title="被评价人" data-index="reviewee_name"/><a-table-column title="得分" data-index="total_score"/><a-table-column title="状态"><template #default="{record}"><a-tag>{{statusLabel(record.status)}}</a-tag></template></a-table-column><a-table-column title="操作" :width="80"><template #default="{record}"><a-tooltip v-if="record.status==='VALID'" title="作废评价"><a-button danger type="text" shape="circle" @click="openInvalidate(record)"><DeleteOutlined/></a-button></a-tooltip></template></a-table-column></a-table>
            </section>
          </template>
          ...
          <template v-else-if="selectedCampaign.assignment_snapshot_at">
          ...
          <template v-else><section class="assignment-pane"><a-alert class="review-scope-alert" type="info" show-icon message="历史量表互评"/><a-list :data-source="candidates"><template #renderItem="{item}"><a-list-item><a-list-item-meta :title="item.name" :description="item.submitted?'已正式提交':'尚未正式提交'"/><a-button v-if="item.submitted&&(!item.reviewed||selectedCampaign.allow_update)" type="primary" @click="startReview(item)">{{item.reviewed?'修改评价':'评价作品'}}</a-button><a-tag v-else-if="item.reviewed" color="green">已评价</a-tag></a-list-item></template></a-list></section></template>
        </section>
      </template>
